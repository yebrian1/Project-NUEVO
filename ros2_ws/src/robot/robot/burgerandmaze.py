...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        # ── DISCRETE COMMAND: MAZE NAVIGATION ─────────────────────────────
            ...
            # Generate the specific waypoints: fwd 2300, turn 90R, fwd 610, turn 90R
        ...
        # ── EXECUTION LOOP: MAZE DRIVING ──────────────────────────────────
            ...
            # Update internal remaining path tracking
            remaining_path = robot._advance_remaining_path(remaining_path, current_x, current_y, advance_radius_mm=50.0)
            
            if not remaining_path:
                print("[ACTION] Maze path exhausted. Stopping.")
                robot.stop()
                state = "IDLE"
                continue

            # Compute velocities (PurePursuitPlanner handles negative linear automatically)
            ...
            if planner1.CurrentTargetReached(target_x, target_y, current_x, current_y):
                print("[ACTION] Maze target reached! Returning to IDLE.")
                robot.stop()
                state = "IDLE"
