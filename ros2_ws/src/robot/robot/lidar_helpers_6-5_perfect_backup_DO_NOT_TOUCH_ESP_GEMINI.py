...
def get_distance_at_angle(points: np.ndarray, angle_deg: float, fov_deg: float =8.0) -> tuple[float | None, int]:
    """
...
def robot_align_to_wall(robot: Robot, poll_angle: float, target_heading: float, timeout_s: float = 15.0) -> bool:
    """
    Blocking helper that performs smooth PD-control alignment to a wall.
    Returns True if success, False if aborted or wall lost.
    """
    start_time = time.monotonic()
    align_max_speed = 10.0 #keep at 10
    align_tolerance = 5.0 #keep at 5
    align_r2_threshold = 0.7 # Very strict for high confidence
    align_fail_count = 0
    align_fail_limit = 20 # Be patient with the sensor
    last_known_heading = 999.0
    
    Kp_align = 1.0 # P-gain
    Kd_align = 0.3 # D-gain (dampens rotation as we approach target)
    
    prev_error = 0.0
    prev_time = None
    
    print(f"[ALIGN] Target: {target_heading}° | Sector: {poll_angle}° (PD Control)")
    ...
    while (time.monotonic() - start_time) < timeout_s:
        ...
        if res and res['r2'] > align_r2_threshold:
            ...
            angular_vel = (error * Kp_align) + (d_error * Kd_align)
            ...
            if np.random.rand() < 0.10:
                print(f"  [ALIGN] Head:{heading:+.1f}° | Err:{error:+.1f}° | D_Err:{d_error:+.1f} | R2:{res['r2']:.2f}")
        else:
            # 4. Handle lost wall / finishing
            robot.stop() 
            align_fail_count += 1
            prev_time = None # Reset derivative on loss of wall
