    ...
    while True:
        ...
        if state == "INIT":
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.move_forward(400.0, velocity=VELOCITY_MM_S, tolerance=2.0, blocking=False)
            elif drive_handle.is_finished():
                robot.cancel_motion()
                drive_handle = None
                state = "POST_NAV_SCAN"
            ...
            gender = classifier.get_gender(wait_for_face=25.0)
            if gender:
                print(f"[RESULT] CUSTOMER IDENTIFIED: {gender}")
                if "girl" in gender.lower():
                    LAPF_GOAL_MM = (70.0, -1500.0)
                else: # boy
                    LAPF_GOAL_MM = (70.0, -1800.0)
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.move_forward(10.0, velocity=VELOCITY_MM_S, tolerance=2.0, blocking=False)
            elif drive_handle.is_finished():
                robot.cancel_motion()
                drive_handle = None
