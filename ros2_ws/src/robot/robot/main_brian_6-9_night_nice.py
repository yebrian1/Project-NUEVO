...
def run(robot: Robot) -> None:
    ...
    # State variables for velocity command throttling
    last_cmd_vel = 0.0
    last_cmd_ang = 0.0
    last_cmd_time = 0.0
    CMD_RATE_LIMIT_S = 0.08  # Send velocity updates max 10Hz to save UART bandwidth

    def set_velocity_throttled(v, w):
        nonlocal last_cmd_vel, last_cmd_ang, last_cmd_time
        now_ts = time.monotonic()
        # Send if values changed OR if enough time has passed
        if (v != last_cmd_vel or w != last_cmd_ang or (now_ts - last_cmd_time) >= CMD_RATE_LIMIT_S):
    ...
    while True:
        ...
        if state == "FIRMWARE_ERROR":
            ...
            # Allow user to clear error with BTN_9
            if robot.was_button_pressed(Button.BTN_9):
                print("[FSM] Attempting to clear error and return to IDLE...")
                robot.reset_estop()
                time.sleep(0.5)
                if robot.get_state() != FirmwareState.ERROR:
                    state = "IDLE"
                    ...
                    print("[FSM] Error clear failed. Check UART/hardware.")
            time.sleep(0.1)
            continue
        ...
        if state == "INIT":
            ...
            if dist is not None and count > 0:
                ...
                set_velocity_throttled(VELOCITY_MM_S, angular_cmd)
                
                if np.random.rand() < 0.1:
                    print(f"[FOLLOW] Dist:{dist:.1f}mm | Err:{error_mm:+.1f}mm | Traveled:{dist_traveled:.0f}mm")
            else:
                set_velocity_throttled(VELOCITY_MM_S, 0.0)
            ...
            #want to raise while drivivng back
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.move_backward(60.0, velocity=VELOCITY_MM_S, tolerance=2.0, blocking=False)
            elif drive_handle.is_finished():
                robot.stop()
                drive_handle = None
                state = "TURN_RIGHT_90"
        ...
        #         state = "DRIVE_FWD_150_2_POST"
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.move_forward(FIRST_LEFT_FORWARD+100, velocity=VELOCITY_MM_S, tolerance=2.0, blocking=False)
            elif drive_handle.is_finished():
                robot.stop()
                drive_handle = None
            ...
            if robot.step_move(ARM_STEPPER, steps=ARM_UP_STEPS * 2, blocking=False, timeout=30.0):
        ...
        #         state = "DRIVE_FWD_150_2_END"
            ...
            seq_step = 0
            start_pose = robot.get_odometry_pose()
            print(f"[FSM] Starting Maze Wall-Follow (2600mm, Target: 330mm)")
            ...
            MAZE_FOLLOW_TARGET_MM = 250.0
            MAZE_FOLLOW_DISTANCE_MM = 2130.0
            
            if dist is not None and count > 0:
                error_mm = dist - MAZE_FOLLOW_TARGET_MM
                angular_cmd = error_mm * FOLLOW_KP
                angular_cmd = max(-20.0, min(20.0, angular_cmd))
                set_velocity_throttled(VELOCITY_MM_S * 2, angular_cmd)
                
                if now - last_status_print_at >= STATUS_PRINT_INTERVAL_S:
                    print(f"[MAZE_FOLLOW] Dist:{dist:.1f}mm | Err:{error_mm:+.1f}mm | Traveled:{dist_traveled:.0f}/{MAZE_FOLLOW_DISTANCE_MM}mm")
                    last_status_print_at = now
            else:
                set_velocity_throttled(VELOCITY_MM_S * 2, 0.0)

            if dist_traveled >= MAZE_FOLLOW_DISTANCE_MM:
                robot.stop()
                print(f"[FSM] Maze Wall-follow complete. Aligning with wall...")
                show_idle_leds(robot)
            ...
            if seq_step == 0:
                ...
                drive_handle = robot.turn_by(-89.0, blocking=True)
            ...
            if seq_step == 0:
                ...
                drive_handle = robot.move_forward(350.0, velocity=VELOCITY_MM_S, tolerance=10.0, blocking=False)
                seq_step = 1
            elif seq_step == 1:
                if drive_handle is not None and drive_handle.is_finished():
                ...
                robot_align_to_wall(robot, 90.0, 0.0)
                drive_handle = robot.move_forward(2600.674206969, velocity=VELOCITY_MM_S*2, tolerance=50.0, blocking=False)
                seq_step = 3
            elif seq_step == 3:
                if drive_handle is not None and drive_handle.is_finished():
            ...
            if seq_step == 0:
                ...
                if drive_handle is not None and drive_handle.is_finished():
                    ...
                    drive_handle = robot.move_forward(50.0, velocity=VELOCITY_MM_S, tolerance=20.0, blocking=False)
                    seq_step = 4
            elif seq_step == 4:
                if drive_handle is not None and drive_handle.is_finished():
                    robot.stop()
        ...
        #         state = "POST_NAV_ALIGN"
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.turn_by(-88.0, blocking=False)
            elif drive_handle.is_finished():
                robot.cancel_motion()
                drive_handle = None
            ...
            if f_dist is not None:
                final_front_dist = f_dist
                target_fwd = final_front_dist - 500.0
                if target_fwd > 0:
            ...
            if results:
                ...
                dropoff_distance_mm = 800.0 if "boy" in gender.lower() else 500.0
                stop_sign_wait_time_s = .7 if "boy" in gender.lower() else 2.0
            ...
            if drive_handle is None:
                ...
                drive_handle = robot.turn_by(-82.0, blocking=False)
            elif drive_handle.is_finished():
                robot.stop()
                drive_handle = None
                start_pose = robot.get_odometry_pose()
            ...
            if dist is not None and count > 0:
                WALL_FOLLOW_TARGET_MM = 350.0
                error_mm = dist - WALL_FOLLOW_TARGET_MM
                angular_cmd = error_mm * FOLLOW_KP
                # Clamp angular speed
                angular_cmd = max(-20.0, min(20.0, angular_cmd))
                
                # Slow down to 300mm/s (Maze speed) for stability
                set_velocity_throttled(VELOCITY_MM_S * 3, angular_cmd)
            ...
            if drive_handle is None:
                target_fwd = final_front_dist - 60.0
                ...
                robot.stop()
                drive_handle = None
                state ="DELIVER_LOWER_ARM"

        elif state == "DELIVER_LOWER_ARM":
            print("[ACTION] Lowering arm for delivery (700 steps)...")
            robot.step_enable(Stepper.STEPPER_1)
            if robot.step_move(Stepper.STEPPER_1, steps=-ARM_UP_STEPS, blocking=True, timeout=20.0):
                state = "DELIVERY_OPEN_GRIPPER"
            else:
                print("[ERROR] Stepper error during delivery arm lower.")
                state = "DELIVERY_OPEN_GRIPPER"
            ...
            if robot.step_move(Stepper.STEPPER_1, steps=ARM_UP_STEPS * 2.3, blocking=True, timeout=20.0):
            ...
            if drive_handle is None:
                print("[ACTION] Driving back 50mm")
                drive_handle = robot.move_backward(40.0, velocity=VELOCITY_MM_S, tolerance=5.0, blocking=False)
            elif drive_handle.is_finished():
                robot.stop()
                drive_handle = None
            ...
            if drive_handle is None:
                print("[ACTION] Turning Right 90°")
                drive_handle = robot.turn_by(-86.0, blocking=False)
            elif drive_handle.is_finished():
                robot.stop()
                drive_handle = None
        ...
        #     robot.step_enable(Stepper.STEPPER_1)
        #     if robot.step_move(Stepper.STEPPER_1, steps=-ARM_UP_STEPS * 3, blocking=True, timeout=20.0):
        #         state = "WATCH_FOR_STOP_SIGN"
        #     else:
        ...
        #         state = "IDLE"
            ...
            if now - stop_sign_timer >= stop_sign_wait_time_s:
                print("[ACTION] " + str(stop_sign_wait_time_s) + " seconds elapsed. Turning camera to 25° while moving.")
            ...
            # Continue forward motion for 1 second
            if now - stop_sign_timer >= 2.0:
                #drive forward 700
                print("[ACTION] Continuing forward motion for 700mm.")
                drive_handle = robot.move_forward(800.0, velocity=VELOCITY_MM_S, tolerance=10.0, blocking=False)
                state = "FINISHED"
            ...
            if drive_handle is not None and drive_handle.is_finished():
                robot.stop()
                drive_handle = None
                print("[FSM] Sequence complete! Returning to IDLE.")
                show_idle_leds(robot)
                state = "IDLE"
