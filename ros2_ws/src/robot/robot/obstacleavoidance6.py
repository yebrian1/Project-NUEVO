...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        if state == "INIT":
            ...
            # Lane Detection and Progress tracking
            current_lane = robot.planner.current_lane
            if current_lane != last_lane:
                if last_lane is not None:
                    print(f"*** LANE SWITCH! Moving from {last_lane} to {current_lane} ***")
                    is_switching_lane = True
                    start_lane_x = pose[0]
                last_lane = current_lane

            if is_switching_lane:
                # Target X mapping for progress calculation
                x_L = robot.planner.x_L
                off = robot.planner.offset
                lane_targets = {'Center': x_L, 'Left': x_L - off, 'Right': x_L + off}
                target_x = lane_targets.get(current_lane, x_L)
                
                dist_to_target = abs(pose[0] - target_x)
                total_switch_dist = abs(target_x - start_lane_x)
                
                if total_switch_dist > 5.0:
                    progress = min(100.0, max(0.0, (1.0 - dist_to_target / total_switch_dist) * 100.0))
                else:
                    progress = 100.0
                
                if is_status_tick:
                    print(f"  [LANE] Switching... {progress:4.1f}% to {current_lane}")
                
                # --- 40% SWITCH COMMITMENT LOCK ---
                if progress < 40.0:
                    # Force the planner to stay in its current avoidance state 
                    # until we've made significant progress into the new lane.
                    robot.planner.avoidance_counter = max(robot.planner.avoidance_counter, 1)
                
                if dist_to_target < 30.0:
                    print(f"✅ CONFIRMED: Robot established in {current_lane} lane (x={pose[0]:.0f}mm)")
                    is_switching_lane = False
