...
def run(robot: Robot) -> None:
    ...
    while True:
        ...
        # ── DISCRETE COMMAND: SEQUENCE MOVE ───────────────────────────────
            ...
            print("[ACTION] Executing Maneuver: Back 200 -> Right 90 -> Forward 200 -> Left 90 -> Forward 200")
            
            # 1. Move backward 200 mm
            robot.move_backward(distance=200.0, velocity=100.0, tolerance=20.0, blocking=True)
            time.sleep(3.5)
            
            # 2. Turn 90 degrees to the right (Clockwise = Negative)
            robot.turn_by(delta_deg=-90.0, blocking=True)
            time.sleep(3.5)
            
            # 3. Move forward 200 mm
            robot.move_forward(distance=200.0, velocity=100.0, tolerance=20.0, blocking=True)
            time.sleep(3.5)
            
            # 4. Turn 90 degrees to the left (Counter-Clockwise = Positive)
            robot.turn_by(delta_deg=90.0, blocking=True)
            time.sleep(3.5)
            
            # 5. Move forward 200 mm
            robot.move_forward(distance=200.0, velocity=100.0, tolerance=20.0, blocking=True)
